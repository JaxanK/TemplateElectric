...
;AutoVersioning
(def directory "I:\\My Drive\\Designer\\Program\\TemplateElectric\\bin")
(def path-to-ver-file "C:\\Workspace\\TemplateElectric\\src\\TemplateElectric\\version.clj")

(def latest-version (av/get-latest-version directory))
(av/update-version-file! latest-version path-to-ver-file)
...
(defn build-client ; invoke with `clj -X ...`
  ...
  (let [{:keys [optimize debug verbose]
    ...
    (b/delete {:path "resources/public/TemplateElectric/js"})
...
(defn uberjar
  ...
  (b/copy-dir {:target-dir class-dir :src-dirs ["src" "src-prod" "resources" "ExternalLibraries"]})
  (let [jar-name (or (some-> jar-name str) ; override for Dockerfile builds to avoid needing to reconstruct the name
        ...
        jar-name (if (str/includes? jar-name "*autoversion*") (str/replace jar-name "*autoversion*" latest-version))
        aliases [:prod]]
...
;; clj -X:build:prod uberjar :build/jar-name "TemplateElectric-*autoversion*-standalone.jar"
